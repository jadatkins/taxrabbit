...
elseif (empty($expense['date']) or strcmp($expense['date'], FINALDATE) >= 0) {
  $delete_button = '

      <div class="deletebutton">
        <input type="submit" name="btn_delete[expense' .
$user_id . '-' . $business_id . '-' . $expense_id . ']" value="Delete" />
      </div>
'; }
...
?>
      ...
      <p>You cannot modify this expense because it is too far in the past.</p>
<?php }
...
if (! $new) echo ' value="', $expense['expense_title'], '"';
if ($read_only) echo ' readonly'; ?> /></td>
	</tr>
...
echo $expense_id; ?>][category]" onchange="exp_enab_mile<?php echo $esuf; ?>()">
            <option<?php if ($descr_id === NULL) echo ' selected';
elseif ($read_only) echo ' disabled'; ?> value=""></option>
<?php
...
while ($cat_descr = $result->fetch_assoc()) {
  ...
  echo '              <option';
  if ($descr_id == $cat_descr['descr_id']) echo ' selected';
  elseif ($read_only) echo ' disabled';
...
echo $expense_id; ?>][disallowable]"<?php
if ($disallow) echo ' checked'; unset($disallow);
if ($read_only) echo ' onclick="return false"';
?> /><label for="tick<?php echo $esuf; ?>"> Disallowable (Personal)</label>
	  </td>
	</tr>
...
echo $expense_id; ?>][date]" size="14"<?php
if (empty($could_be_blank)) echo ' required="required"'; ?> min="<?php
...
if ($read_only) echo ' readonly'; ?> /> miles</td>
                </tr>
              </table></fieldset>
...
if (! $new) {
  ...
  print_system_dates($expense, $esuf);
}
?>
